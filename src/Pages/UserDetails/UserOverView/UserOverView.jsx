...
import { FaGraduationCap, FaHome, FaShoppingBag, FaPhone, FaCloudversify, FaBirthdayCake } from 'react-icons/fa';
...
const UserOverView = () => {
    ...
    const userVersity = userEducations?.filter(edu => edu.category === 'university')
            ...
            <div className='px-6 py-6'>
                ...
                { workexperians && <div className='flex justify-between items-center mt-2'>
                    <div className='flex gap-10'>
                        ...
                        <div className='flex '>
                            {
                                workexperians?.map(work => <h1 key={work?._id} className='font-sans'><span className='mr-1'>{work?.position}</span>at<span className='mr-1 ml-1  text-blue-500'>{work?.company}</span>,</h1>)
                            }
                        </div>
                    </div>
                ...
                {userVersity && <div className='flex justify-between items-center mt-2'>
                    <div className='flex gap-10'>
                        ...
                        <div>
                            {
                                userVersity?.slice(0, 1)?.map(education => <h1 key={education?._id} className='font-sans'>Studies at <span className='text-blue-500'>{education?.university} </span>From  {education?.startyear} <span> to </span>{education?.endyear}<span className='ml-1 text-blue-500'>{education?.studentStatus}</span></h1>)}
                        </div>
                    </div>
                    <div className="dropdown dropdown-top dropdown-end">
                        <label tabIndex={0} className="btn btn-circle btn-sm text-[20px] font-sans font-bold bg-gray-200  pb-4 mt-2">...</label>
                ...
                <div className='flex justify-between items-center mt-2'>
                    <div className='flex gap-10'>
                        ...
                        <h1 className=' font-sans'>Marride Status <span className='text-blue-500'>Single</span></h1>
                    </div>
                    <div className="dropdown dropdown-top dropdown-end">
                        <label tabIndex={0} className="btn btn-circle btn-sm text-[20px] font-sans font-bold bg-gray-200  pb-4 mt-2">...</label>
                        <ul tabIndex={0} className="dropdown-content z-[1] menu p-2 shadow bg-white-300 rounded-box w-52">
