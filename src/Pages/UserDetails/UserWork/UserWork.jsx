...
const UserWork = () => {
    ...
    const [singleexperians, setSingleexperians] = useState([]);
    const getdatawork = singleexperians[0];
    ...
    // id base find data
    const getsingleWork = (id) => {
         setId(id);
         axiosSequre.get(`/getwork/${id}`)
        .then(data => {
            setSingleexperians(data?.data);
        }).catch(err => {
            console.log(err);
        })
    }

    // user work experians update 
    const updateExperians = (event) => {
        ...
        const updateWork = { company, position, location, description, workstatus, startyear, endYear };
        console.log(updateWork);
        axiosSequre.put(`updatework/${id}`, updateWork)
            .then(data => {
                if (data?.data.modifiedCount) {
                    refetch();
                    Swal.fire('updated works experians');
                }
            })
    };


    // work experians delate 
    const workExperiansDelate = (id) => {
        Swal.fire({
            title: "Are you sure?",
            ...
            if (result?.isConfirmed) {
                axiosSequre.delete(`/deletework/${id}`)
                    .then((data) => {
    ...
    return (
        <div className='px-10 py-10'>
                        ...
                        workexperians?.map(experians => <div key={experians?._id} className='flex justify-between items-center mt-4'>
                            ...
                            <div className="dropdown dropdown-top dropdown-end">
                                ...
                                <ul tabIndex={0} className="dropdown-content z-[1] menu p-2 shadow bg-white-300 rounded-box w-52">
                                    <li><label htmlFor="my_modal_18" onClick={() => getsingleWork(experians?._id)} className='btn w-full btn-success pt-4 text-white mb-3'>Edit Project</label></li>
                                    <li><button onClick={() => workExperiansDelate(experians?._id)} className='btn w-full btn-error pt-4 text-white '>Delet Project</button></li>
                                </ul>
                            </div>
            ...
            {/* update user work experians  */}
            <div>
                <input type="checkbox" id="my_modal_18" className="modal-toggle" />
                <div className="modal">
                    <div className="modal-box">
                        <div className="modal-action">
                            <label htmlFor="my_modal_18" className="btn btn-circle bg-green-400 absolute top-3 right-3 text-white">X</label>
                        </div>
                        <form onSubmit={updateExperians}>
                            {/* company  */}
                            <div className="form-control">
                                <label className="label">
                                    <span className="label-text">Company</span>
                                </label>
                                <input defaultValue={getdatawork?.company} type="text" placeholder="type here" className="input input-bordered" />
                            </div>
                            {/* positions  */}
                            <div className="form-control">
                                <label className="label">
                                ...
                                <input name='position' defaultValue={getdatawork?.position} type="text" placeholder="type here" className="input input-bordered w-full" />
                            </div>
                            {/* city/town*/}
                            <div className="form-control">
                                <label className="label">
                                ...
                                <input name='location' defaultValue={getdatawork?.location} type="text" placeholder="type here" className="input input-bordered w-full" />
                            </div>
                            {/* description*/}
                            <div className="form-control">
                                <label className="label">
                                ...
                                <textarea name='description' defaultValue={getdatawork?.description} className="textarea textarea-bordered w-full" placeholder=""></textarea>
                            </div>
                            ...
                            <div className="form-control">
                                ...
                                <select name='workstatus' defaultValue={getdatawork?.workstatus} className="select border  border-green-500  w-full ">
                                    <option disabled selected>select</option>
                                    <option>yes</option>
                                    <option>No</option>
                                </select>
                            ...
                            <div className="form-control">
                                ...
                                <div className='flex justify-center items-center gap-5 '>
                                    <input name='startyear' defaultValue={getdatawork?.startyear} type="number" placeholder="year" className="input input-bordered w-full" />
                                    <p className='font-sans font-bold'>To</p>
                                    <input name='endYear' defaultValue={getdatawork?.endYear} type="number" placeholder="year" className="input input-bordered w-full" />
                                </div>
                            </div>
                            <div className="form-control mt-5">
                                <input type="submit" value="Save" className="btn bg-green-500 text-white hover:bg-green-500" />
