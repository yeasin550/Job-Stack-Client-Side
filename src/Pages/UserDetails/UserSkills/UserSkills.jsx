...
const UserSkills = () => {
  ...
  const updateSkill = (id) => {
    openModal();
    setClickedSkill(id);
    axiosSequre.get(`/skill/${id}`).then((data) => {
  ...
  const singleskillUpdate = (event) => {
    ...
    axiosSequre.put(`/updateskills/${clickedSkill}`, skillnew).then((data) => {
  ...
  const skillDelet = (id) => {
    Swal.fire({
      ...
      if (result?.isConfirmed) {
        axiosSequre.delete(`/skill/${id}`).then((data) => {
    ...
    <div className="px-10 py-10">
      <div className="flex flex-col shadowdiv border p-4 rounded-md">
        ...
        <div className="flex justify-start items-center gap-2 mt-2 mb-2">
          ...
          <label htmlFor="my_modal_4" className="link text-blue-500">
            Add Skills
          </label>
        </div>
        {userSkills?.map((userskill) => (
          ...
          >
            ...
            <div className="flex gap-2">
              <button
                onClick={() => updateSkill(userskill?._id)}
                className="btn btn-xs bg-green-500 hover:bg-green-700 text-white"
              >
                Edit
              </button>
              <button
                onClick={() => skillDelet(userskill?._id)}
                className="btn btn-xs bg-red-600 hover:bg-rose-700 text-white"
              >
                Delet
              </button>
            </div>
          ...
          <div className="modal-box">
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
              ...
              <div className="form-control">
                ...
                <select
                  {...register("skillrate", { required: true })}
          ...
          <div method="dialog" className="modal-box relative">
            <form onSubmit={singleskillUpdate}>
              <div className="form-control mt-6">
                <label className="label">
                  <span className="label-text">
                    Your Skill You Provide Current SkillRate
                  </span>
                </label>
                <input
                  type="text"
                  readOnly
                  defaultValue={singleskill[0]?.skillrate}
                  placeholder="type here"
                  className="input input-bordered"
                />
                <select
                  name="skillrate"
                  id="skillrate"
                  className="select border  border-green-500 mt-6 mb-5 w-full "
                >
                  <option disabled selected>
                    select
                  </option>
              ...
              <input
                type="submit"
                value="Save Changes"
                className="w-full py-2 mt-3 bg-green-500 rounded-md text-white cursor-pointer"
              ></input>
            </form>
            {/* modal close button  */}
            <div className="modal-action">
