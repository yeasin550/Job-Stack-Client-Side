...
const UserProjectSkills = () => {
  ...
  const { data: projectskills = [], refetch } = useQuery(
    ["projectskills"],
    async () => {
  ...
  const deletProject = (id) => {
    Swal.fire({
      ...
      if (result?.isConfirmed) {
        axiosSequre.delete(`/deletproject/${id}`).then((data) => {
    ...
    <div className="px-5 py-5">
      <div className="flex flex-col shadowdiv border p-4 rounded-md">
        ...
        <div className="flex justify-start items-center gap-2 mt-2 mb-2">
          ...
          <label onClick={openModal} className="link text-blue-500">
            Add Project
          </label>
        </div>
          ...
          >
            <div className="flex flex-col justify-start items-start">
              <h1 className="font bold font-sans text-2xl">
                {projectskill?.projectTitle}
              </h1>
              <h1 className="link text-blue-500 font-bold font-sans">
                <span>{projectskill?.workduration}-</span>
                {projectskill?.duration}
              </h1>
              <p>{projectskill?.projectDescription}</p>
              <h1 className="link text-blue-500 font-bold font-sans">
                {projectskill?.liveLink}
              </h1>
              <h1 className="link text-blue-500 font-bold font-sans">
                {projectskill?.codeLink}
              </h1>
            </div>
            <div className="flex justify-end gap-2 mt-3">
              <button
                onClick={() => navigate(`/projectupdate/${projectskill?._id}`)}
                className="btn btn-sm bg-green-500 hover:bg-green-700 text-white"
              >
                Edit
              </button>
              <button
                onClick={() => deletProject(projectskill?._id)}
                className="btn btn-sm bg-red-600 hover:bg-rose-700 text-white"
              >
                Delet
              </button>
            </div>
        ...
        >
          ...
          >
            <div className="modal-action">
              <label
                onClick={closeModal}
                className="btn btn-circle absolute right-4 top-3 bg-green-400 text-white"
              >
                X
              </label>
            </div>
            ...
            <div className="form-control">
              ...
              <input
                ...
                type="url"
                pattern="https://.*"
                placeholder="https://codeLink.com"
                className="input input-bordered"
              />
              {errors.codeLink && (
                <span className="text-purple-600 animate-pulse">
                  codeLink is required
                </span>
              )}
            </div>
