...
const UserFollowing = () => {
    ...
    const { refetch, data: followings = [] } = useQuery( ["followings", user?.email], async () => {
            ...
            {followings ? (
                <div className="grid md:grid-cols-3 gap-7   px-5 w-full h-screen overflow-auto">
                    {followings?.map((person) => (
                        <div key={person?._id} className="w-full h-72 bg-white shadow-2xl  rounded-lg relative">
                            <div className="">
                                <img
                                    ...
                                    draggable="false"
                                />
                                <div onClick={() => navigate(`/dynamicprofile/${person?._id}`)} className="flex justify-center cursor-pointer -mt-12">
                                    {person?.userImage ? (
                                        <img
                                        ...
                                        <div
                                            className="h-[100px] w-[100px] bg-gray-200 p-4 rounded-full">
